...
char **argv;
{
    ...
    case 2:
        ...
        {
            fprintf(fileOut, "0x%04X%s", (unsigned int)(buffer[i] * 256 + buffer[i + 1]), separator);
            chars_in_line += stringlength;
            if (chars_in_line >= wrap_line)
            {
                fprintf(fileOut, "\n");
        ...
        fprintf(fileOut, "0x%04X", (unsigned int)(buffer[i] * 256 + buffer[i + 1]));
        break;
    case 4:
        stringlength = 10 + strlen(separator);
        for (i = 0; i < lSize - 4; i += 4)
        {
            fprintf(fileOut, "0x%08X%s", (unsigned int)(buffer[i] * 16777216 + buffer[i + 1] * 65536 + buffer[i + 2] * 256 + buffer[i + 3]), separator);
            chars_in_line += stringlength;
            if (chars_in_line >= wrap_line)
            {
                fprintf(fileOut, "\n");
        ...
        fprintf(fileOut, "0x%08X", (unsigned int)(buffer[i] * 16777216 + buffer[i + 1] * 65536 + buffer[i + 2] * 256 + buffer[i + 3]));
        break;
    default:
